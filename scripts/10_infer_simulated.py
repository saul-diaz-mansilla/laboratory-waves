...
def main():
    parser = argparse.ArgumentParser(description="Inverse-problem inference (simulated override)")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Simulation index to evaluate from the dataset.",
    )
    ...
    df_results = io.load_parquet_data(simulation_dir, prefix="results_")
    df_freqs = io.load_parquet_data(simulation_dir, prefix="freqs_")
    df_targets = io.load_parquet_data(simulation_dir, prefix="targets_")
    
    if idx < 0 or idx >= len(df_results):
        raise ValueError(f"Requested index {idx} out of range (0-{len(df_results)-1}).")

    freqs_exp = np.asarray(df_freqs.iloc[idx]["freqs_global"])
    ...
    if model_type == "2d":
        ...
        for i, node in enumerate(target_nodes):
            ...
            if key_mag in df_results.columns and key_phase in df_results.columns:
                H_mag = np.asarray(df_results.iloc[idx][key_mag])[sort_idx_exp]
                H_phase = np.asarray(df_results.iloc[idx][key_phase])[sort_idx_exp]
            else:
                print(f"Warning: Node {node} data missing from simulated results. Padding with 0.")
        ...
        model.load_state_dict(torch.load(ckpt, map_location=device))
        model.eval()
        H_mag = np.asarray(df_results.iloc[idx][f"H_Mag_{node_1d}"])[sort_idx_exp]
        H_phase = np.asarray(df_results.iloc[idx][f"H_Phase_{node_1d}"])[sort_idx_exp]
        fm = interp1d(
    ...
    H_sim_mag_target = np.asarray(df_results.iloc[idx][key_mag])[sort_idx_exp]

    # Calculate ground truth parameters from the target dataframe
    C_true_raw = np.asarray(df_targets["C_norm"].iloc[idx])
    L_true_raw = np.asarray(df_targets["L_norm"].iloc[idx])
    C_true = C_true_raw * C_0
    L_true = L_true_raw[:n_circ - 1] * L_0

    for tag, C_custom, L_custom in runs:
        print(f"\n--- Inference ({tag}) on Simulated Row {idx} ---")
        print(f"Mean True L:      {np.mean(L_true) * 1e6:.2f} uH")
        print(f"Mean Predicted L: {np.mean(L_custom) * 1e6:.2f} uH")
        print(f"Mean True C:      {np.mean(C_true) * 1e9:.2f} nF")
        ...
        safe = tag.replace("/", "_")
        out_plot = f"figures/10_infer_simulated_{safe}.png"
