...
timestamp, output_voltage, output_current, thermistor_temperatures, comments = (
    load_dataset("archive/thermal/data/task_1_6_" + str(int(T)) + ".csv")
)
...
popt, pcov = spo.curve_fit(
    sine_f,
    timestamp,
    thermistor_temperatures[:, 0],
    p0=[0, w, np.mean(thermistor_temperatures[:, 0]), 1, 0.0008],
    bounds=(
...
for i in range(8):  # 0.0008
    ...
    popt, pcov = spo.curve_fit(
        ...
        p0=[phase_guess, w, np.mean(thermistor_temperatures[:, i]), 0.5, 0.0008],
        bounds=(
            [-np.inf, 0, -np.inf, 0, -np.inf],
            [np.inf, np.inf, np.inf, np.inf, np.inf],
        ),
