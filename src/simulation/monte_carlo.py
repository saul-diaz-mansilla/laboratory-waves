...
import src.simulation.ode_solver as solver
...
def simulate(config_path, L_custom=None, C_custom=None):
    ...
    # When custom parameters are provided (inference mode), only run 1 simulation
    if L_custom is not None or C_custom is not None:
        num_simulations = 1
