...
timestamp, output_voltage, output_current, thermistor_temperatures, comments = (load_dataset(os.path.join(os.path.dirname(__file__), "../../data/thermal/task_1_5.csv")))
...
gamma = []
for i in range(8):
    popt, pcov = spo.curve_fit(sine_f, timestamp, thermistor_temperatures[:, i], p0=[6, w, 32, .5, 0.0008], bounds=([0, 0, -np.inf, 0, -np.inf], [2*np.pi, np.inf, np.inf, np.inf, np.inf]))
    if i==0:
        phi0 = popt[0]
        a0 = popt[3]
