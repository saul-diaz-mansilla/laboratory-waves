...
freq_exp = w / (2 * np.pi)
popt_exp, pcov_exp = opt.curve_fit(linear_func, freq_exp, ratio)
m_exp = popt_exp[0]
dm_exp = np.sqrt(pcov_exp[0, 0])

print(f"Experimental Slope: {m_exp:.6e} +/- {dm_exp:.6e}")
...
for sim_i in range(n_simulations):
    ...
    # Fit for this simulation
    popt_sim, pcov_sim = opt.curve_fit(linear_func, frequencies, ratio_V40_V0)
    sim_slopes.append(popt_sim[0])
    sim_slope_errs.append(np.sqrt(pcov_sim[0, 0]))

sim_slopes = np.array(sim_slopes)
sim_slope_errs = np.array(sim_slope_errs)

m_sim_mean = np.mean(sim_slopes)
m_sim_std = np.std(sim_slopes)
m_sim_err_max = np.max([m_sim_std, np.max(sim_slope_errs)])

ratio_val = m_sim_mean / m_exp
ratio_err = ratio_val * np.sqrt(
    (m_sim_err_max / m_sim_mean) ** 2 + (dm_exp / m_exp) ** 2
)

print(f"Simulated Slope Mean: {m_sim_mean:.6e}")
print(f"Simulated Slope Std: {m_sim_std:.6e}")
print(f"Max Sim Error: {m_sim_err_max:.6e}")
print(f"Ratio m_sim/m_exp: {ratio_val:.6f} +/- {ratio_err:.6f}")
