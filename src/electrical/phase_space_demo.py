...
def main():
    ...
    v_in_phase = (
        amplitude * np.sin(omega * t + 0.01 * random.random()) * random.random()
    )

    # Wave 2: Out of phase (dphi = pi)
    v_out_phase = (
        amplitude * np.sin(omega * t + np.pi + 0.01 * random.random()) * random.random()
    )

    # Wave 3: pi/2 out of phase (dphi = pi/2)
    v_quadrature = (
        amplitude
        * np.sin(omega * t + (np.pi / 2) + 0.3 * random.random())
        * random.random()
    )
    ...
    plt.plot(v0, v_in_phase, label=r"In Phase ($\Delta\phi=0$)", linewidth=2)
    ...
    plt.plot(v0, v_quadrature, label=r"Other $\Delta\phi$", linestyle="--", linewidth=2)

    plt.xlabel(r"$V_{in}$ (V)", fontsize=16)
    plt.ylabel(r"$V_{out}$ (V)", fontsize=16)
